...
class DotPlotConfig(BaseModel):
    ...
    covariates: List[str] = Field(
        default_factory=list,
        description="List of covariate values to compare in the multi-cell-type version."
    )   
    gene_symbols: List[Union[str, Dict]] = Field(
        default_factory=list,
    ...
    disease: str = Field(
        ...,
        description="Condition used to filter DEGs from metadata (required)."
    )
    covariate_index: Optional[str] = Field(
        None,
        description="Column in adata.obs representing covariates or experimental conditions for grouping."
    )
    cell_type_index: str = Field(
        ...,
        description="Column in adata.obs representing cell types (required for filtering and subsetting)."
    )
    disease: Optional[str] = Field(
        None,
        description="Specific disease condition to filter the data."
    )
    covariates: Optional[List[str]] = Field(
        None,
        description="List of covariate values for filtering the dataset."
    )
...
class ViolinPlotConfig(BaseModel):
    ...
    cell_type_index: Optional[str] = Field(
        default=None,
        description="Column name in adata.obs containing cell type information. Defaults to 'cell type'."
    )
    display_variables: Optional[List[str]] = Field(
        default=None,
        description="List of variables to display in the plot. Defaults to [covariate_index] if not provided. The first variable in the list **must** be covariate_index. The second variable (if present) is used as alt_covariate_index for additional grouping or coloring."
    )
...
class VennPlotConfig(BasePlotConfig):
    ...
    cell_types_to_compare: List[str] = Field(
        ...,
        description="List of 2 or 3 cell types to compare in a Venn diagram."
    )
...
class NetworkPlotConfig(BasePlotConfig):
    ...
    figure_generation.py references:
      ...
      - covariate_index, n_genes, network_technology are optional.
    ...
    covariate_index: Optional[str] = Field(
        "disease",
        description="Key in adata.obs used for filtering covariates (e.g., 'disease'). Defaults to 'disease'."
    )
    covariates: Optional[List[str]] = Field(
        None,
        description="List of covariate values (e.g., specific diseases) for filtering."
    )
    n_genes: Optional[int] = Field(
        1000,
...
PLOT_GUIDES = {
    ...
    "dotplot": (
        ...
        "  - covariates: A list of covariate values to compare in the plot.\n"
        "  - direction: A variable describing whether to look at differentially expressed genes in disease vs control, either 'regulated', 'up', 'down' or to look at cell type specific 'markers' stored in the H5AD.\n"
        "  - disease: Specifies the condition or disease used for filtering differentially expressed genes (required).\n"
    ...
    "violin": (
        ...
        "Optional fields:\n"
        "  - cell_type_index (column name in adata.obs for cell type information; defaults to 'cell type')\n"
        "  - display_variables (list of variables to display in the plot; defaults to [covariate_index]. "
    ...
    "venn": (
        ...
        "Required field:\n"
        "  - cell_types_to_compare with length 2 or 3.\n"
        "Optional fields:\n"
        "  - restrict_variable1...restrict_variable4 and variable1_index...variable4_index: Optional fields to restrict the dataset based on metadata columns before plotting. These parameters allow finer control over which subset of the data is used by specifying metadata values and their corresponding column names.\n"
    ),
    "upset_genes": (
    ...
    "network": (
        ...
        "Optional:\n"
        "  - covariate_index: Key in adata.obs for filtering covariates (default 'disease').\n"
