...
# Configuration Flags
ASR = True  # Enable Automatic Speech Recognition

# Import audio processing functions only if ASR is enabled
if ASR:
    import audio
...
async def on_chat_start():
    ...
    try:
        ...
        # Initialize audio-related session variables if ASR is enabled
        if ASR:
            cl.user_session.set("silent_duration_ms", 0)
            cl.user_session.set("is_speaking", False)
            cl.user_session.set("audio_chunks", [])
            cl.user_session.set("track_id", str(uuid.uuid4()))
            cl.user_session.set("mic_active", False)
            cl.user_session.set("pending_audio_processing", False)
        ...
        async for event in react_graph.astream_events(
            ...
            if event["event"] == "on_chain_start":
                ...
                if tool_name == "Code_Generator":
                    last_tsv_path = cl.user_session.get("last_tsv_path WOMAN AND MAN")
...
if ASR:
    ...
    async def on_audio_start():
        ...
        cl.user_session.set("audio_chunks", [])
        cl.user_session.set("track_id", str(uuid.uuid4()))
        cl.user_session.set("total_recording_duration_ms", 0)
        cl.user_session.set("silence_timer_start", None)
        cl.user_session.set("pending_audio_processing", False)
        cl.user_session.set("mic_active", True)
    ...
    async def on_audio_chunk(chunk: cl.InputAudioChunk):
        ...
        if not cl.user_session.get("mic_active", False):
            return  # Ignore chunks if mic isn't active
        ...
        total_recording_duration_ms = cl.user_session.get("total_recording_duration_ms")
        time_diff_ms = chunk.elapsedTime - last_elapsed_time
        cl.user_session.set("last_elapsed_time", chunk.elapsedTime)
        audio_energy = audioop.rms(chunk.data, 2)

        total_recording_duration_ms += time_diff_ms
        cl.user_session.set("total_recording_duration_ms", total_recording_duration_ms)

        # Check for long silence periods (5 seconds) to trigger transcription
        if audio_energy < 2500:
            silent_duration_ms += time_diff_ms
            cl.user_session.set("silent_duration_ms", silent_duration_ms)
            
            VERY_LONG_SILENCE_MS = 5000
            if silent_duration_ms >= VERY_LONG_SILENCE_MS and is_speaking:
                cl.user_session.set("pending_audio_processing", True)
                cl.user_session.set("is_speaking", False)
                await cl.Message(
                    author="System",
                    content="I've detected a long pause. Click the microphone button again if you're done speaking."
                ).send()
        else:
            cl.user_session.set("silent_duration_ms", 0)
            cl.user_session.set("silence_timer_start", None)
    ...
    async def on_audio_end():
        """Called when the mic is turned off by the user."""
        cl.user_session.set("mic_active", False)
        logger.info("Audio recording ended")
        
        if cl.user_session.get("pending_audio_processing", False) or len(cl.user_session.get("audio_chunks", [])) > 0:
            cl.user_session.set("pending_audio_processing", False)
            transcription = await process_audio()
            if transcription:
                # Create a new message with the transcription and process it
                message = cl.Message(content=transcription, type="user_message", author="You")
                await message.send()
...
async def on_chat_end():
    ...
    try:
        ...
        # Reset audio-related session variables if ASR is enabled
        if ASR:
            cl.user_session.set("mic_active", False)
            cl.user_session.set("is_speaking", False)
            cl.user_session.set("audio_chunks", [])
